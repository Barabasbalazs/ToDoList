<template>
  <div class="text-center border-2 border-black mx-3.5 rounded-2xl h-[82px] lg:h-[163.32px]">
    <div
    ...
    >
      ...
      <div>
        <div class="p-1 rounded-full h-fit"
            :class="[(priorityOfThisItem === Priority.low) ? 'bg-blue' : '', 
            (priorityOfThisItem === Priority.medium) ? 'bg-yellow' : '',
            (priorityOfThisItem === Priority.high) ? 'bg-red' : '']"></div>
      </div>
    </div>
    <div class="flex flex-col m-[18px] font-semibold hidden lg:block justify-between">
        <div class="flex flex-row justify-between items-center">
            <p class="text-4xl">{{ props.item.title }}</p>
            <div class="text-lg text-white rounded-full"
            :class="[(priorityOfThisItem === Priority.low) ? 'bg-blue' : '', 
            (priorityOfThisItem === Priority.medium) ? 'bg-yellow' : '',
